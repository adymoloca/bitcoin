...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/flocoin.conf are parsed in qt/flocoin.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (!args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo()) + "\n"
                "\nUsage:  flocoind [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see flocoind -h for a list of options.\n", argv[i])));
            }
        }
        ...
        // -server defaults to true for flocoind but not for the GUI so do this here
...
int main(int argc, char* argv[])
{
    ...
    // Connect flocoind signal handlers
    noui_connect();
