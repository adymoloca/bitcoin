// Copyright (c) 2015-2020 The Flocoin Core developers
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<FlocoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_flocoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_flocoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_flocoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = FlocoinUnits::formatWithUnit(unit, balance, false, FlocoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    FlocoinAmountField* amountInput = receiveCoinsDialog.findChild<FlocoinAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_flocoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
