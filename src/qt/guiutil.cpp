...
#include <qt/flocoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new FlocoinAddressEntryValidator(parent));
    widget->setCheckValidator(new FlocoinAddressCheckValidator(parent));
}
...
bool parseFlocoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no flocoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!FlocoinUnits::parse(FlocoinUnits::FLO, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseFlocoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseFlocoinURI(uriInstance, out);
}

QString formatFlocoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("flocoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(FlocoinUnits::format(FlocoinUnits::FLO, info.amount, false, FlocoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openFlocoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", FLOCOIN_CONF_FILENAME));
    ...
    /* Open flocoin.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("flocoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a flocoin.desktop file to the autostart directory:
