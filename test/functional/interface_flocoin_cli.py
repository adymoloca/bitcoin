...
# The block reward of coinbaseoutput.nValue (50) FLO/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 FLO/block.
...
class TestFlocoinCli(FlocoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `flocoin-cli getblockchaininfo`")
        ...
        if self.is_wallet_compiled():
            self.log.info("Test -getinfo and flocoin-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (FLO/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (FLO/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 FLO) to the default wallet.
            ...
            # Test flocoin-cli -generate with -rpcwallet in multiwallet mode.
