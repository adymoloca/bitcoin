...
// flocoin includes.
#include <..\src\script\flocoinconsensus.h>
...
int main()
{
    std::cout << "flocoinconsensus version: " << flocoinconsensus_version() << std::endl;
    ...
    auto op0Result = flocoinconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, stream.data(), stream.size(), 0, flocoinconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
